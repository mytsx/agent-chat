...
- **`agent_chat.read.id_ranges`** — okumanın döndürdüğü mesajların tam kümesi, bitişik `[başlangıç,bitiş]` çiftleri olarak. `ReadMessages` limit dolduğunda yalnızca en yeni kuyruğu döndürür, dolayısıyla en yüksek kimlik alttakilerin görüldüğünü kanıtlamaz: eski bir doğrudan mesaj yeni broadcast'lerce dışarı itilebilir. İlerleme bir **küme**dir, watermark değil. Okumalar pratikte bitişik kuyruk olduğu için bu kodlama tek çifte iner — dolayısıyla bir üst sınıra ve "sınırı aştı, watermark'a düş" yedeğine hiç gerek kalmaz. Bu önemli: manager ve observer join'leri odanın truncation'ını bilerek atlıyor, yani oda 500'ü aşabiliyor ve `read_all_messages(limit=1000)` sabit her sınırı geçebiliyor. `read.message_ids` ve `read.max_id` yalnızca daha eski akışlar için okunur.
- **`agent_chat.room.generation`** — mesajın saklandığı ya da okumanın gördüğü kuşak, **oda kilidi altında** damgalanır. Kuşağı olayın sınır kaydına göre nerede durduğundan çıkarmak yarışlıdır: eşzamanlı bir clear ile yarışan gönderim, sakladıktan sonra loglandığı için yanlış tarafa düşer.
