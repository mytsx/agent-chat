...
// sanitizeNotificationPrompt strips terminal-control payloads from notification text
// before it is injected into an agent PTY. Agent names are validated on the normal
// team-store path, but messages can also be replayed from disk/imports; a hostile
// From/To value must not be able to close bracketed-paste mode or send live control
// keys through the auto-poke path.
